...
export function Profile() {
  ...
  // Cálculos locais
  const healthMetrics = user 
    ? (() => {
        // Cálculos simulados (apenas para UI se domain falhar, embora não devia)
        return {
          bmi: 25,
          bmiLabel: 'Normal',
          bmr: 1600,
  ...
  // Lógica local de resumo (se user existir)
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-gray-900">
              {user.goal === 'maintain' ? 'Manter peso' : user.goal === 'loss_moderate' ? 'Emagrecer (Moderado)' : 'Emagrecer (Acelerado)'}
            </span>
          </div>
        </div>
      </section>

      {/* 3. SAÚDE & METAS (RESTAURADA) */}
      <section>
        <div className="flex justify-between items-center mb-3">
          ...
          <button 
            ...
            title="Ver explicações sobre IMC, TMB, Kcal, etc."
          >
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col justify-between border-emerald-200">
               <div>
                 ...
                 <p className="text-[9px] font-medium text-emerald-700 mt-1">
                   {user.goal === 'loss_aggressive' ? 'Acelerado' : user.goal === 'loss_moderate' ? 'Moderado' : 'Manutenção'}
                 </p>
                 {/* Botão Info (Removido por enquanto para manter limpo) */}
               </div>
            </div>
          </div>
        )}
